...
const Testimonials = () => {
  ...
  const nextSlide = () => {
    setIndex((prevIndex) =>
      prevIndex < testimonialsData.length - visibleCards ? prevIndex + 1 : prevIndex
    );
  };

  return (
    <div className="flex py-6 md:py-8 flex-col items-center justify-center gap-6 bg-[#19A45B]">
      {/* Navigation */}
      <div className="flex items-center justify-between w-full max-w-[343px] sm:max-w-[1200px] px-4 md:px-8 lg:px-12">
        <button onClick={prevSlide} className="p-3 md:p-4 bg-white border border-[#C7D3E1] rounded-xl cursor-pointer">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="19" viewBox="0 0 18 19" fill="none">
            <path d="M15 9.31055H3M3 9.31055L7.5 13.8105M3 9.31055L7.5 4.81055" stroke="#D0D0D0" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
        <div className="py-3 px-6 rounded-4xl border border-[#00802F] bg-[#19A45B] text-white font-circular-normal leading-6">
          Testimonials
        </div>
        <button onClick={nextSlide} className="p-3 md:p-4 bg-[#001A00] rounded-xl cursor-pointer">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="19" viewBox="0 0 18 19" fill="none">
            <path d="M3.75 9.31055H14.25M14.25 9.31055L9 4.06055M14.25 9.31055L9 14.5605" stroke="white" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
      </div>
      ...
      <div className="relative flex justify-center items-center md:items-start w-full max-w-[343px] sm:max-w-[1200px] overflow-hidden md:gap-6">
        <AnimatePresence>
          {testimonialsData.slice(index, index + visibleCards).map((testimonial) => (
            <motion.div
              key={testimonial.id}
            ...
            >
              <div className="flex flex-col gap-5">
                <div className="flex items-center gap-3">
                  <Image src={testimonial.image} width={48} height={48} alt="avatar" />
                  <div className="flex flex-col">
                    <p className="text-base font-medium">{testimonial.name}</p>
                    <p className="text-sm text-[#42526B]">{testimonial.company}</p>
                  </div>
                </div>
                <p className="text-base font-light text-[#061C3D]">{testimonial.review}</p>
      ...
      <div className="flex gap-2.5">
        {testimonialsData.map((_, idx) => (
          <div key={idx} className={`w-3 h-3 rounded-full ${index === idx ? 'bg-[#001A00] w-8' : 'bg-[#CCEBDB]'}`} />
        ))}
      </div>
    </div>
  );
